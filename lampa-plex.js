...
(function () {
    ...
    // левое меню, см. openLibrary) свежим экземпляром компонента — он сам
    // заново запросит список у Plex, удалённый тайтл в нём уже не придёт,
    // отдельно «обновлять» нечего.
    //
    // Первая версия дёргала Lampa.Activity.replace(..., true), рассчитывая
    // ПОДМЕНИТЬ текущую (уже неактуальную) карточку в стеке вместо того,
    // чтобы добавлять новую активность поверх. На практике это не сработало
    // молча — пользователь подтвердил, что удаление и индикатор отработали
    // штатно, а перехода в библиотеку не случилось вовсе, без единой ошибки.
    // Это указывает, что `replace` либо не является публичным методом
    // Lampa.Activity в реальном приложении, либо ведёт себя иначе, чем в
    // изученном исходнике — а брошенное внутри .then() исключение тихо
    // проглатывалось следующим .catch(). Возвращаемся к Lampa.Activity.push —
    // ровно тому же вызову, которым уже пользуется openLibrary() из левого
    // меню (и он точно работает). Плата за надёжность — «назад» из свежей
    // библиотеки ведёт на удалённую карточку (она просто больше не покажет
